...
cs_user_model(void)
{
  ...
  // Characteristic macroscopic length of the domain, used for the initialization of the turbulence
  // (taken equal to the hydraulic diameter) NO
  // (taken equal to GTS length
  glob_turb_ref_values->almax = 0.0073951; //0.01331118; //0.021135083; //2.47648095; //2.510117647; //0.439630802;
  glob_turb_ref_values->uref = 94.244;
  //cs_turb_model_log_setup(); // Dio error usarla
   ...
   */
  {
    ...
    //wf->iwallf = CS_WALL_F_2SCALES_CONTINUOUS;
	wf->iwallf = CS_WALL_F_2SCALES_LOG;
  }
...
//    fp->p0 = 1.01325e5;
    ...
    /* We only specify XYZ0 if we explicitely fix Dirichlet conditions
       for the pressure. */
	// Information extracted from "GTS press tap.txt" archive provided by Storms (tap no. 106)
	// Consider that Storms uses a different system of reference
	// Considering that width at x=(-7.647+4.4706)w_gts ~ -1.028 is 3.06208281 m
// 	  cs_lnum_t w_gts = 0.32385;
//    fp->xyzp0[0] = -7.647*w_gts + 4.4706*w_gts;
//    fp->xyzp0[1] = 1.530; //1.524; it was out 
//    fp->xyzp0[2] = 2.5882*w_gts;
