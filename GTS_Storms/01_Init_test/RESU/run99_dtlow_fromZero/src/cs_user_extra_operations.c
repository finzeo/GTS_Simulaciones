...
cs_user_extra_operations(cs_domain_t     *domain)
{
    ...
    //if (b_forces != NULL) {
     ...
     cs_selector_get_b_face_list("gtsModel or gtsStrutsPads", &n_elts, lst_elts);
      //cs_selector_get_b_face_list("gtsModel or gtsStruts or gtsStrutsPads", &n_elts, lst_elts);
      // Another example:
	...
	// Whether struts are considered (or not) in the calculation of the projected area
	bool strutsInForce = false;
    ...
    for (int i=0;i<n_yaw_angle_values;i++){
      if( (yaw_angle-yaw_angle_values[i])<tol_read && (yaw_angle-yaw_angle_values[i])>(-tol_read) ){
		if(strutsInForce){
			...
			switch(i){
			  case 0: proj_area = cs_notebook_parameter_value_by_name("proj_area_0_0_wostruts");break;
			  case 1: proj_area = cs_notebook_parameter_value_by_name("proj_area_2_5_wostruts");break;
			  case 2: proj_area = cs_notebook_parameter_value_by_name("proj_area_5_0_wostruts");break;
			  case 3: proj_area = cs_notebook_parameter_value_by_name("proj_area_7_5_wostruts");break;
			  case 4: proj_area = cs_notebook_parameter_value_by_name("proj_area_10_0_wostruts");break;
			  case 5: proj_area = cs_notebook_parameter_value_by_name("proj_area_12_5_wostruts");break;
			  case 6: proj_area = cs_notebook_parameter_value_by_name("proj_area_14_0_wostruts");break;
			  default: {fprintf(stderr, "Projected area not found. Exiting...\n"); exit(-1);}
			}
		}
		break;
    ...
    cs_selector_get_b_face_list("gtsModel or gtsStrutsPads", &n_elts_p, lst_elts_p);
    //cs_selector_get_b_face_list("gtsModel or gtsStruts or gtsStrutsPads", &n_elts_p, lst_elts_p);
  ...
  // (name set at snappyHexMeshDict)
  // Creo que se pueden usar operadores logicos
  cs_selector_get_b_face_list("gtsModel or gtsStruts or gtsStrutsPads", &n_elts_y, lst_elts_y);
...
/*
	// Detecting p0 prescribed position
      // Number of boundary faces
      const cs_lnum_t n_b_faces_3 = domain->mesh->n_b_faces;
	  // pressure is a face scalar field
	  cs_field_t *pressure_b = cs_field_by_name_try("pressure");
      // Number and array of boundary faces (to use only certain parts,
      // containing boundary faces that meet a certain criteria)
      cs_lnum_t n_elts_3, *lst_elts_3;
      // Allocate memory for n_b_faces elements of type cs_lnum_t,
      // in lst_elts pointer (to fill a list of boundary faces)
      BFT_MALLOC(lst_elts_3, n_b_faces_3, cs_lnum_t);
      // Fill a list of boundary faces verifying a given selection criteria
      // For this case, using a group name of mesh entities, only faces on GTS
      // (name set at snappyHexMeshDict)
      // Creo que se pueden usar operadores lógicos
     cs_selector_get_b_face_list("outlet", &n_elts_3, lst_elts_3);
      //cs_selector_get_b_face_list("gtsModel or gtsStruts or gtsStrutsPads", &n_elts, lst_elts);
      // Another example:
      // cs_selector_get_b_face_list 
      // ("normal[0, -1, 0, 0.1] and box[-1000, -1000, -1000, 1000, 0.01, 1000]",
      ...
      // Loop over GTS boundary faces
      for (cs_lnum_t i_elt_3 = 0; i_elt_3 < n_elts_3; i_elt_3++) {
        cs_lnum_t face_id = lst_elts_3[i_elt_3];
		cs_lnum_t tol = 1e-4;
		if (pressure_b->val[face_id] > -tol && pressure_b->val[face_id] < tol) printf("face id 0 Pa: %d \n",face_id);
		if (pressure_b->val[face_id] < -5000) printf("face id -5000 Pa: %d \n",face_id);
      }
      // Free resources
      BFT_FREE(lst_elts_3);
*/
