                         ...
                         const char       *condition)
{
  ...
  if (strcmp(field_name, "direction") == 0 &&
    ...
    for (cs_lnum_t e_id = 0; e_id < n_elts; e_id++) {
      ...
      ang = atan(y * 0.445333 / (inlet_width * 100.0 / 2.0));
  ...
  if (strcmp(field_name, "turbulence_kw") == 0 &&
    ...
    const cs_real_t vel_inlet = cs_notebook_parameter_value_by_name("vel_inlet");

    cs_real_t I = -1.;
    for (cs_lnum_t e_id = 0; e_id < n_elts; e_id++) {
      cs_lnum_t b_e_id = elt_ids[e_id];
      I = turbIntensity;
      new_vals[0 * n_elts + e_id] = 1.5 * cs_math_pow2((I * vel_inlet));
      new_vals[1 * n_elts + e_id] = (pow(new_vals[0 * n_elts + e_id], 0.5)) / 0.0073951;
    }
  }
  return new_vals;
}
